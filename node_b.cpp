    ...
    void imageCallback(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        for(int i = 1; i < num_labels; i++) // Start from 1 to skip background
        {
			...
			if (depthInit && intrinsicParamInit) {
				...
				float depth = latest_depth_image.at<float>(y_int, x_int);  // Access depth at (y, x)
				ROS_INFO_STREAM("Depth of point at (" << x_int << ", " << y_int << ") = " << depth);

				// Compute 3D position in the camera frame
				...
				camera_point.header.stamp = ros::Time(0);  // Use the latest available transform
				...
				try {
					...
					// Print the centroid's position in the map frame
					ROS_INFO("Map coordinates of red centroid: [x: %f, y: %f, z: %f]",
						     map_point.point.x, map_point.point.y, map_point.point.z);

					// Update navigation goal with the transformed position
					geometry_msgs::PoseStamped newGoal;
					newGoal.header.frame_id = "map";
					newGoal.header.stamp = ros::Time::now();
					newGoal.pose.position.x = map_point.point.x;
					newGoal.pose.position.y = map_point.point.y;
					newGoal.pose.orientation.w = 1.0;  // Default orientation

					navigation_goals.push_back(newGoal);
				} catch (tf2::TransformException &ex) {
					ROS_WARN("Transform failed for red centroid: %s", ex.what());
				}
			}
